...
struct SoundConfig {
    ...
    SoundConfig() : soundFile(""), playMode("loop") {}
    SoundConfig(const std::string& file, const std::string& mode) : soundFile(file), playMode(mode) {}
};
...
void CleanStopFiles() {
    ...
    std::vector<std::string> stopFiles = {"C:\\temp\\stop_OSoundtracks_Base.tmp",
                                          "C:\\temp\\stop_OSoundtracks_Menu.tmp",
                                          "C:\\temp\\stop_OSoundtracks_Specific.tmp"};
...
void SendDebugSoundBeforeFreeze(ScriptType type, ScriptState& scriptState) {
    ...
    logger::info("Sending Debug.wav to {} before freeze (500ms active playback)", scriptState.name);
    ...
    logger::info("Debug.wav playback completed for {}, ready to freeze cleanly", scriptState.name);
...
void ClassifyAnimations() {
    ...
    logger::info("Classified animations - Base: {}, Specific: {}, Menu: {}", g_baseTracks.size(),
                 g_specificTracks.size(), g_menuTracks.size());
...
void CleanOldScripts() {
    ...
    try {
        ...
        logger::info("Cleaned {} old PowerShell scripts from Key_OSoundtracks", deletedCount);
...
void GenerateBaseScript() {
    ...
    script << "$stopFile = 'C:\\temp\\stop_OSoundtracks_Base.tmp'\n";
    script << "$trackFile = 'C:\\temp\\track_OSoundtracks_Base.tmp'\n";
    ...
    g_baseScript.stopFilePath = "C:\\temp\\stop_OSoundtracks_Base.tmp";
    g_baseScript.trackFilePath = "C:\\temp\\track_OSoundtracks_Base.tmp";

    logger::info("Generated Base script");
...
void GenerateSpecificScript() {
    ...
    script << "$carpetaSonidos = '" << soundsFolder << "'\n";
    script << "$archivoLog = '" << heartbeatPath << "'\n";
    script << "$stopFile = 'C:\\temp\\stop_OSoundtracks_Specific.tmp'\n";
    script << "$trackFile = 'C:\\temp\\track_OSoundtracks_Specific.tmp'\n";
    script << "$tiempoEsperaSegundos = 6\n\n";
    ...
    g_specificScript.stopFilePath = "C:\\temp\\stop_OSoundtracks_Specific.tmp";
    g_specificScript.trackFilePath = "C:\\temp\\track_OSoundtracks_Specific.tmp";

    logger::info("Generated Specific script");
...
void GenerateMenuScript() {
    ...
    script << "$carpetaSonidos = '" << soundsFolder << "'\n";
    script << "$archivoLog = '" << heartbeatPath << "'\n";
    script << "$stopFile = 'C:\\temp\\stop_OSoundtracks_Menu.tmp'\n";
    script << "$trackFile = 'C:\\temp\\track_OSoundtracks_Menu.tmp'\n";
    script << "$tiempoEsperaSegundos = 6\n\n";
    ...
    g_menuScript.stopFilePath = "C:\\temp\\stop_OSoundtracks_Menu.tmp";
    g_menuScript.trackFilePath = "C:\\temp\\track_OSoundtracks_Menu.tmp";

    logger::info("Generated Menu script");
...
void GenerateCheckScript() {
    ...
    logger::info("Generated Check script (Watchdog)");
...
void GenerateStaticScripts() {
    ...
    logger::info("Generated 4 static PowerShell scripts successfully");
...
void StartAllScriptsFrozen() {
    ...
    WriteToSoundPlayerLog("Starting all scripts in frozen state...", __LINE__);
...
// ===== PLAY SOUND IN SCRIPT (MODIFIED TO ALWAYS SEND COMMAND) =====
void PlaySoundInScript(ScriptType type, const std::string& soundFile) {
...
void StopScript(ScriptState& scriptState) {
    ...
    logger::info("Stopping script: {} (isPaused: {}, currentTrack: '{}')",
                 scriptState.name, scriptState.isPaused, scriptState.currentTrack);
    ...
    if (scriptState.processInfo.hProcess != 0 && scriptState.processInfo.hProcess != INVALID_HANDLE_VALUE) {
        ...
        logger::info("Script {} terminated directly (no unfreeze)", scriptState.name);
    ...
    logger::info("Script {} stopped cleanly (forced termination)", scriptState.name);
...
void StopAllScripts() {
    ...
    logger::info("All scripts stopped successfully");
    WriteToSoundPlayerLog("All scripts stopped", __LINE__);
}
...
bool LoadIniSettings() {
    try {
        ...
        logger::info("INI settings loaded - Volume: {}%, Startup Sound: {}", g_soundVolume.load(),
                     g_startupSoundEnabled.load() ? "enabled" : "disabled");
    ...
    try {
        ...
        while (currentPos < content.length()) {
            ...
            if (commaPos != std::string::npos && arrayEnd != std::string::npos && commaPos < arrayEnd) {
                size_t modeQuoteStart = content.find('"', commaPos);

                if (modeQuoteStart != std::string::npos && modeQuoteStart < arrayEnd) {
                    size_t modeQuoteEnd = content.find('"', modeQuoteStart + 1);

                    if (modeQuoteEnd != std::string::npos && modeQuoteEnd < arrayEnd) {
                        playMode = content.substr(modeQuoteStart + 1, modeQuoteEnd - modeQuoteStart - 1);

                        std::transform(playMode.begin(), playMode.end(), playMode.begin(), ::tolower);

                        if (playMode != "loop" && playMode != "once") {
                            logger::warn("Invalid play mode '{}' for animation '{}', defaulting to 'loop'", playMode,
                                         animationName);
                            playMode = "loop";
                        }
                    }
            ...
            g_animationSoundMap[animationName] = SoundConfig(soundFile, playMode);

            logger::info("Loaded sound mapping: {} -> {} [{}]", animationName, soundFile, playMode);
        ...
        logger::info("Loaded {} sound mappings from JSON", g_animationSoundMap.size());
        ...
        for (const auto& [anim, config] : g_animationSoundMap) {
            WriteToSoundPlayerLog("  " + anim + " -> " + config.soundFile + " [" + config.playMode + "]", __LINE__);
        }
...
void PlayStartupSound() {
    ...
    if (startIt != g_animationSoundMap.end()) {
        logger::info("PLAYING STARTUP SOUND: {}", startIt->second.soundFile);
...
void PlaySound(const std::string& soundFileName, bool waitForCompletion) {
    try {
        ...
        logger::info("Playing sound: {}", soundPath.string());
    ...
    if (animationName != g_currentSpecificAnimation) {
        ...
        if (!g_currentSpecificAnimation.empty()) {
            ...
            if (prevSpecificIt != g_animationSoundMap.end()) {
                ...
                logger::info("Cleaning previous specific sound '{}' with Debug.wav before playing '{}'",
                             prevSpecificIt->second.soundFile, specificIt->second.soundFile);
...
public:
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESActivateEvent* event,
        ...
        if (event && event->actionRef.get() && event->objectActivated.get()) {
            auto activator = event->actionRef.get()->GetBaseObject();
            auto activated = event->objectActivated.get()->GetBaseObject();
            if (activator && activated) {
                std::stringstream msg;
                msg << activator->GetName() << " activated " << activated->GetName();
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESCombatEvent* event,
        ...
        if (event && event->actor.get() && event->targetActor.get()) {
            auto actor = event->actor.get()->As<RE::Actor>();
            auto target = event->targetActor.get()->As<RE::Actor>();
            if (actor && target) {
                std::string combatState = (event->newState.underlying() == 0) ? "stopped" : "started";
                std::stringstream msg;
                msg << "Combat " << combatState << " between " << actor->GetName() << " and " << target->GetName();
                WriteToActionsLog(msg.str(), __LINE__);
            }
        }
        return RE::BSEventNotifyControl::kContinue;
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESContainerChangedEvent* event,
        ...
        if (event && event->baseObj) {
            auto form = RE::TESForm::LookupByID(event->baseObj);
            if (form) {
                std::string action = event->itemCount > 0 ? "added" : "removed";
                std::stringstream msg;
                msg << action << " " << std::abs(static_cast<int>(event->itemCount)) << " item(s) of "
                    << form->GetName();
                WriteToActionsLog(msg.str(), __LINE__);
            }
        }
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESFurnitureEvent* event,
        ...
        if (event && event->actor.get() && event->targetFurniture.get()) {
            auto actor = event->actor.get()->As<RE::Actor>();
            auto furniture = event->targetFurniture.get();
            if (actor && furniture) {
                std::string action =
                    (event->type == RE::TESFurnitureEvent::FurnitureEventType::kEnter) ? "entered" : "exited";
                std::stringstream msg;
                msg << actor->GetName() << " " << action << " furniture " << furniture->GetName();
                WriteToActionsLog(msg.str(), __LINE__);
            }
        }
        return RE::BSEventNotifyControl::kContinue;
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESHitEvent* event, RE::BSTEventSource<RE::TESHitEvent>*) override {
        if (event && event->cause.get() && event->target.get()) {
            auto attacker = event->cause.get()->As<RE::Actor>();
            auto target = event->target.get()->As<RE::Actor>();
            if (attacker && target) {
                std::stringstream msg;
                msg << attacker->GetName() << " hit " << target->GetName() << " for damage";
                WriteToActionsLog(msg.str(), __LINE__);
            }
        }
        return RE::BSEventNotifyControl::kContinue;
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESQuestStageEvent* event,
        ...
        if (event) {
            std::stringstream msg;
            msg << "Quest stage " << static_cast<int>(event->stage) << " reached for quest ID " << std::hex
                << std::setfill('0') << std::setw(8) << event->formID;
            WriteToActionsLog(msg.str(), __LINE__);
        }
        return RE::BSEventNotifyControl::kContinue;
    }
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESSleepStartEvent* event,
        ...
        WriteToActionsLog("Player started sleeping", __LINE__);
        return RE::BSEventNotifyControl::kContinue;
    }
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESSleepStopEvent* event,
        ...
        if (event) {
            std::stringstream msg;
            msg << "Player stopped sleeping (interrupted: " << (event->interrupted ? "yes" : "no") << ")";
            WriteToActionsLog(msg.str(), __LINE__);
        }
        return RE::BSEventNotifyControl::kContinue;
    }
    ...
    RE::BSEventNotifyControl ProcessEvent(const RE::TESWaitStopEvent* event,
        ...
        if (event) {
            std::stringstream msg;
            msg << "Player stopped waiting (interrupted: " << (event->interrupted ? "yes" : "no") << ")";
            WriteToActionsLog(msg.str(), __LINE__);
        }
        return RE::BSEventNotifyControl::kContinue;
    }
    ...
    try {
        ...
        if (!g_initialDelayComplete) {
            ...
            if (elapsedSeconds < 5) {
                ...
                logger::info("5-second initial delay complete, starting OStim.log monitoring");
        ...
        while (std::getline(ostimLog, line)) {
            ...
            if (line.find("[Thread.cpp:634] closing thread") != std::string::npos) {
                isThreadStop = true;
                logger::info("DETECTED: OStim thread closing");
                ...
                isThreadStop = true;
                logger::info("DETECTED: OStim trying to stop thread");
            ...
            if (isAnimationLine && !animationName.empty()) {
                ...
                if (!g_firstAnimationDetected) {
                    logger::info("First animation detected, reloading JSON mappings and regenerating scripts...");
...
void MonitoringThreadFunction() {
    logger::info("Monitoring thread started with static script system");
    ...
    if (logsFolder) {
        ...
        WriteToSoundPlayerLog("Waiting 30 seconds before starting OStim.log analysis...", __LINE__);
    }
...
void StartMonitoringThread() {
    if (!g_monitoringActive) {
        ...
        logger::info("MONITORING SYSTEM ACTIVATED WITH STATIC SCRIPT SYSTEM");
...
void InitializePlugin() {
    try {
        ...
        WriteToSoundPlayerLog("OSoundtracks Plugin - v10.0.5 (STATIC SCRIPT EDITION WITH FORCED TERMINATION FIX)", __LINE__);
        ...
        WriteToActionsLog("OSoundtracks Actions Monitor - v10.0.5", __LINE__);
        WriteToActionsLog("Started: " + GetCurrentTimeString(), __LINE__);
        WriteToActionsLog("========================================", __LINE__);
        WriteToActionsLog("Monitoring game events: Activate, Menu, Combat, Container, etc.", __LINE__);
        WriteToActionsLog("", __LINE__);
        ...
        if (LoadSoundMappings()) {
            ...
            logger::info("PLUGIN INITIALIZED WITH STATIC SCRIPT SYSTEM");
...
SKSEPluginLoad(const SKSE::LoadInterface* a_skse) {
    ...
    logger::info("OSoundtracks Plugin v10.0.5 - STATIC SCRIPT EDITION WITH FORCED TERMINATION FIX - Starting...");

    InitializePlugin();

    SKSE::GetMessagingInterface()->RegisterListener(MessageListener);

    logger::info("Plugin loaded successfully with static script system");
...
constinit auto SKSEPlugin_Version = []() {
    ...
    v.PluginVersion({10, 0, 5});
    v.PluginName("OSoundtracks OStim Monitor - Static Script Edition");
